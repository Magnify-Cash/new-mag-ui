...
export function SidebarNav() {
  ...
  const [selectedNetwork, setSelectedNetwork] = React.useState(navigationData.networks[0]);
  ...
  return (
    <Sidebar>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-[--radix-dropdown-menu-trigger-width]">
                {navigationData.networks.map((network) => (
                  <DropdownMenuItem key={network} onSelect={() => setSelectedNetwork(network)}>
                    {network} {network === selectedNetwork && <Check className="ml-auto size-4" />}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <SidebarContent>
        ...
        {isTestnet && navigationData.testnetOnly.map((section) => (
          <SidebarGroup key={section.title}>
            <SidebarGroupLabel>{section.title}</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
